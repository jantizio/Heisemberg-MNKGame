...
public class BoardStatus {
    ...
    private Score[][] rowScore_opponent, columnScore_opponent, mainDiagonalScore_opponent,
            secondaryDiagonalScore_opponent;
    ...
    private void clearScores(int x, int y) {
        if (rowScore_player[x][y] == null && columnScore_player[x][y] == null && mainDiagonalScore_player[x][y] == null
                && secondaryDiagonalScore_player[x][y] == null) {
            return;
        }
    ...
    /**
     * Restituisce un vettore di Score tale che la posizione i-esima indichi il
     * numero di mosse mancanti per vincere se si selezionasse la mossa in quella
     * posizione
     * 
     * @param board        Vettore contenente la configurazione di gioco rispetto ad
     *                     una riga/colonna/diagonale
    ...
    private Score[] getScoresArray(MNKCellState[] board, MNKCellState toCheckState) {
        ...
        for (int i = 1; i < board.length; i++) { // O(board.length)
            if (board[i] == oppositeState) {
                ...
                } // Se sono in grado di allinearne fino al target, imposto l'inizio
                  // dell'allineamento
            } else {
            ...
            if (s[i].aligned == target) {
                for (int k = 1; k <= target - 1 && i - k >= 0; k++) { // O(K)
                    // Interrompo se incontro una mossa migliore della i-esima (necessita di meno
                    // mosse per vincere)
                    if (s[i - k].aligned == target && s[i - k].moves <= s[i].moves) {
                        break;
                    }
    ...
    /**
     * Imposta il punteggio di una determinata cella rispetto ad una determinata
     * direzione.
     * 
     * @implNote Costo: Θ(1)
     */
    ...
    /**
     * Riempie, nella matrice degli Score delle righe, la riga contenente la cella
    ...
    private void fillRowScoreAt(int x, int y) {
        if (rowScore_player[x][y] != null && rowScore_opponent[x][y] != null) {
            return;
        }
    ...
    /**
     * Riempie, nella matrice degli Score delle colonne, la colonna contenente la
     * cella nella posizione indicata
     * 
     * @implNote Costo (pessimo): O(M*K)<br/>
     *           Costo (ottimo): Θ(1)
     */
    private void fillColumnScoreAt(int x, int y) {
        if (columnScore_player[x][y] != null && columnScore_opponent[x][y] != null) {
            return;
        }
    ...
    /**
     * Riempie, nella matrice degli Score delle diagonali principali, la diagonale
     * contenente la cella nella posizione indicata
     * 
     * @implNote Costo (pessimo): O(min{M, N}*K)<br/>
     *           Costo (ottimo): Θ(1)
     */
    private void fillMainDiagonalScoreAt(int x, int y) {
        if (mainDiagonalScore_player[x][y] != null && mainDiagonalScore_opponent[x][y] != null) {
            return;
        }
    ...
    /**
     * Riempie, nella matrice degli Score delle diagonali secondarie, la diagonale
     * contenente la cella nella posizione indicata
     * 
     * @implNote Costo (pessimo): O(min{M, N}*K)<br/>
     *           Costo (ottimo): Θ(1)
    ...
    private void fillSecondaryDiagonalScoreAt(int x, int y) {
        if (secondaryDiagonalScore_player[x][y] != null && secondaryDiagonalScore_opponent[x][y] != null) {
            return;
        }
    ...
    /**
     * Riempie in tutte le matrici degli score la riga/colonna/diagonale che include
     ...
     * 
     * @implNote Costo (pessimo): O(M*N*K) = O(2*4*M*N*K) L'idea è che per generare
     *           tutti gli score, bisogna iterare su tutte le celle disponibili<br/>
     *           Costo (ottimo): Θ(1)
     */
    public void generateGlobalMovesToWin() {
        for (int x = 0; x < columns; x++) {
            fillColumnScoreAt(x, 0);
        } // O(N * M*K)
        ...
        for (int y = 0; y < rows; y++) {
            fillMainDiagonalScoreAt(0, y);
        }
        for (int x = 1; x < columns; x++) {
            fillMainDiagonalScoreAt(x, 0);
        } // O((M+N-1) * min{M, N}*K) dove (M+N-1) è il numero di diagonali

        for (int y = 0; y < rows; y++) {
            fillSecondaryDiagonalScoreAt(columns - 1, y);
        }
        for (int x = columns - 2; x >= 0; x--) {
            fillSecondaryDiagonalScoreAt(x, 0);
        } // O((M+N-1) * min{M, N}*K) dove (M+N-1) è il numero di diagonali
    }
    ...
    private Score[][] rowScoreFilter(MNKCellState toCheckState) {
        if (toCheckState == PLAYER_STATE) {
            return rowScore_player;
        } else {
            return rowScore_opponent;
        }
    }

    private Score[][] columnScoreFilter(MNKCellState toCheckState) {
        if (toCheckState == PLAYER_STATE) {
            return columnScore_player;
        } else {
            return columnScore_opponent;
        }
    }

    private Score[][] mainDiagonalScoreFilter(MNKCellState toCheckState) {
        if (toCheckState == PLAYER_STATE) {
            return mainDiagonalScore_player;
        } else {
            return mainDiagonalScore_opponent;
        }
    }

    private Score[][] secondaryDiagonalScoreFilter(MNKCellState toCheckState) {
        if (toCheckState == PLAYER_STATE) {
            return secondaryDiagonalScore_player;
        } else {
            return secondaryDiagonalScore_opponent;
        }
    }

    /**
     * Restituisce il numero minimo di mosse necessarie per vincere ad una
     * determinata cella
     * 
     * @param toCheckState Indica lo stato della cella che si vuole controllare
     *                     (giocatore o avversario)
     ...
     * @param toCheckState Indica lo stato della cella che si vuole controllare
     *                     (giocatore o avversario)
     * @return Array di interi dove se v[i] = q allora ci sono q modi per vincere
     *         che necessitano di un numero di i mosse
    ...
    public int[] getAllPossibleWinningScenariosCount(MNKCellState toCheckState) {
        ...
        for (int y = 0; y < rows; y++) { // Θ(M)
            ...
            for (int x = 0; x < columns; x++) { // Θ(N)
                if (rowScore[x][y].aligned != target) {
                    continue;
                }
        ...
        for (int x = 0; x < columns; x++) { // Θ(N)
            prevStart = -1;
            for (int y = 0; y < rows; y++) { // Θ(M)
                if (columnScore[x][y].aligned != target) {
                    continue;
                }
        ...
        for (int x = 1; x < columns; x++) { // Θ(N)
            ...
            prevStart = -1;
            while (isValidCell(i, j)) { // O(min{M, N}) [Lunghezza max diagonale]
                if (mainDiagonalScore[i][j].aligned == target && isFreeAt(i, j) && isFreeAt(i, j)
                        && prevStart != mainDiagonalScore[i][j].start) {
                    prevStart = mainDiagonalScore[i][j].start;
                    out[mainDiagonalScore[i][j].moves]++;
                }
    ...
    /**
     * Restituisce un array contenente il numero di possibili modi per vincere
     * rispetto ad una determinata posizione
     * 
     * @param toCheckState Indica lo stato della cella che si vuole controllare
     *                     (giocatore o avversario)
     * @return Array di interi dove se v[i] = q allora ci sono q modi per vincere
    ...
    public int[] getAllPossibleWinningScenariosCountAt(int toCheckX, int toCheckY, MNKCellState toCheckState) {
        ...
        prevStart = -1;
        for (int x = 0; x < columns; x++) { // Θ(N)
            if (rowScore[x][toCheckY].aligned != target) {
                continue;
            }
        ...
        prevStart = -1;
        for (int y = 0; y < rows; y++) { // Θ(M)
            if (columnScore[toCheckX][y].aligned != target) {
                continue;
            }
    ...
    /**
     * Restituisce lo stato della griglia controllando da una determinata posizione
     * (vittoria, sconfitta, pareggio, partita aperta)
     * 
     * @implNote Costo: Θ(1) [GLI SCORE DEVONO ESSERE STATI GENERATI]
     */
