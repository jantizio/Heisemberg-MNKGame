...
public class BoardEnhanced {
    ...
    public BoardEnhanced(int M, int N, int K) {
        ...
        for (int i = 0; i < max_dim; i++)
            this.weightArray[i] = new DoublyLinkedList();
        for (int i = 0; i < M; i++) {
            for (int j = 0; j < N; j++) {
            ...
            for (int y = -1; y <= 1; y++) {
                ...
                for (int k = 1; k < K; k++) {
                    ...
                    if (inBounds(i1, j1)) {
                        ...
                        if (this.nodesBoard[i1][j1].getItem().state == MNKCellState.FREE)
                            this.weightArray[this.nodesBoard[i1][j1].getItem().getWeight()]
                                    .addNode(this.nodesBoard[i1][j1]);
                    }
                }
            }
        }
